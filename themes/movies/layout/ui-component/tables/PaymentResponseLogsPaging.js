...
import {PaymentResponseRowShowMore} from "./rows/PaymentResponseRowShowMore"
// Generate Order Data
export const PaymentResponseLogsPaging = memo(() => {
    const [rowsPerPage, setRowsPerPage] = useState(10)
    const dispatch = useDispatch()
    const history = useHistory()
    let rows = useSelector(state => state.payments.data.response)
    let params = useSelector(state => state.payments.responseParams)
    let startParams = useSelector(state => state.payments.responseStartParams)
    ...
    useEffect(() => {
        if (startParams) {
            ...
            dispatch(allActions.payments.paymentResponseLogs(par))
        }
    ...
    const handleChangePage = (event, newPage) => {
        ...
        if (params.page !== currentPage) {
            dispatch(allActions.payments.setResponseLogParams({...params, page: currentPage}, history))
        }
    }

    const handleChangeRowsPerPage = (event) => {
        setRowsPerPage(+event.target.value)
        dispatch(allActions.payments.setResponseLogParams({...params, page: 1, item_per_page: event.target.value}, history))
    }
            ...
            <Table size="small" aria-label="table" sx={{tableLayout:"fixed"}}>
                ...
                <TableBody>
                    {rows && Array.isArray(rows) && rows.length > 0 ? rows.map((row) => (
                        <PaymentResponseRowShowMore row={row} key={row._id}/>
                    )) : <TableRow>
