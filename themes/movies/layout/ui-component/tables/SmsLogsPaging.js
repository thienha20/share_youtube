...
// Generate Order Data
export const SmsLogsPaging = memo(() => {
    const [rowsPerPage, setRowsPerPage] = useState(10)
    const dispatch = useDispatch()
    const history = useHistory()
    let rows = useSelector(state => state.sms.data)
    let params = useSelector(state => state.sms.params)
    let startParams = useSelector(state => state.sms.startParams)
    ...
    useEffect(() => {
        if (startParams) {
            ...
            dispatch(allActions.sms.smsLogs(par))
        }
    ...
    const handleChangePage = (event, newPage) => {
        ...
        if (params.page !== currentPage) {
            dispatch(allActions.sms.setLogParams({...params, page: currentPage}, history))
        }
    }

    const handleChangeRowsPerPage = (event) => {
        setRowsPerPage(+event.target.value)
        dispatch(allActions.sms.setLogParams({...params, page: 1, item_per_page: event.target.value}, history))
    }
